<#
.SYNOPSIS
  Count aggregate lines across all files under a path.

.DESCRIPTION
  Recursively finds files with specified extensions (skips bin, obj, and api.Tests folders), counts total lines,
...
.EXAMPLE
  pwsh ./.helpers/count-codelines.ps1 -Path . -ExcludeBlank -ExcludeComments -FileTypes "*.cs"
#>
...
# Find files with specified extensions, ignore bin, obj, and api.Tests directories
$files = Get-ChildItem -Path $Path -Recurse -Include $FileTypes -File -ErrorAction SilentlyContinue |
         Where-Object { $_.FullName -notmatch '\\(bin|obj|api.Tests|build|dist|node_modules|ios|android)\\' }
...
Write-Host "Total lines (all" $($FileTypes -join ', ') "files):" $totalLines
