...
export const MarketingCard = ({ eyebrow, title, description, bullets, href, hrefLabel }: MarketingCardProps) => (
  <article className="surface-card rounded-card p-4 text-left transition duration-200 hover:-translate-y-1 hover:shadow-float md:p-7">
    ...
    {href && hrefLabel
      ? (() => {
          ...
          if (isExternal) {
            return (
              <a
                className="mt-6 inline-flex items-center gap-2 text-xs font-semibold text-white transition hover:text-white/80 sm:text-sm"
                href={href}
          ...
          return (
            <Link className="mt-6 inline-flex items-center gap-2 text-xs font-semibold text-white transition hover:text-white/80 sm:text-sm" href={href}>
              {hrefLabel}
              <ArrowRightIcon className="h-4 w-4" />
            </Link>
          );
