  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,280px),minmax(0,1fr)] lg:items-start">
      <aside className="lg:sticky lg:top-2 lg:max-h-[calc(100vh-8rem)] lg:overflow-hidden">
        <div className="surface-card-light border border-slate-150 rounded-[var(--radius-card)] p-6 lg:max-h-[calc(100vh-8rem)] lg:overflow-y-auto lg:overscroll-contain">
          ...
          <details className="mt-6 group">
            ...
            <div className="mt-2 grid gap-2 rounded-[24px] border border-slate-900/10 bg-white p-2">
              {legalPages.map((page) => (
                <Link
                  className={cn(
      ...
      <article className="min-w-0 break-words transition rounded-[var(--radius-card)] border border-white/90 bg-white p-4 shadow-[0_24px_72px_rgba(15,23,42,0.08)] sm:p-8 lg:p-10">
        <div className="border-b border-slate-900/10 pb-4">
          ...
          <h1 className="text-3xl font-semibold tracking-[-0.06em] text-slate-950 sm:text-5xl">{renderInlineNodes(document.title)}</h1>

          {document.meta.length > 0 ? (
            <div className="mt-6 flex flex-col gap-4 border-t border-slate-900/10 pt-5 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="mt-8 space-y-4">{document.blocks.map((block, index) => renderBlock(block, index))}</div>
      </article>
    </div>
  );
};